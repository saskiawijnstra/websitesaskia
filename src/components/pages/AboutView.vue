  ...
  <article class="about">
    ...
    <div class="experiences block grid-container">
      ...
      <dl class="col-span-4-start-1">
        ...
        >
          <component
            :is="getIcon(experience['icon-path'])"
    ...
    <div class="education block grid-container">
      ...
      <dl class="col-span-6-start-6">
        ...
        >
          <component
            :is="getIcon(education['icon-path'])"
...
const icons = import.meta.glob("@/assets/images/experience-icons/*.svg", {
  eager: true,
  import: "default",
...
function normalizeIconName(filename: string) {
  return filename.replace(/\.(png|jpe?g|svg)$/i, "").replace(/\d+$/, "");
}

function getIcon(iconPath: string) {
  const target = normalizeIconName(iconPath.split("/").pop() ?? "");
  const match = Object.entries(icons).find(([path]) => {
    const filename = path.split("/").pop() ?? "";
    return normalizeIconName(filename) === target;
  });

  if (!match) {
    console.warn(`No matching svg icon found for "${iconPath}".`);
  }
...
import WaterlelieIcon from "@/assets/images/experience-icons/waterlelie.svg";
